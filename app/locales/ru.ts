...
const ru: PartialLocaleType = {
    ...
    Settings: {
        ...
        Prompt: {
            ...
            ListCount: (builtin: number, custom: number) => `${builtin} встроенных, ${custom} пользовательских`,
            Edit: 'Редактировать',
            Modal: {
    ...
    Store: {
        ...
        Prompt: {
            History: (content: string) => 'Это краткое содержание истории чата между ИИ и пользователем: ' + content,
    ...
    Mask: {
        ...
        EditModal: {
            Title: (readonly: boolean) => `Редактирование шаблона подсказки ${readonly ? '(только для чтения)' : ''}`,
            Download: 'Скачать',
            Clone: 'Клонировать',
        },
        Config: {

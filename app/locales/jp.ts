...
const jp: PartialLocaleType = {
    ...
    Settings: {
        ...
        Prompt: {
            ...
            List: 'カスタムプロンプトリスト',
            ListCount: (builtin: number, custom: number) => `組み込み ${builtin} 件、ユーザー定義 ${custom} 件`,
            Edit: '編集',
            Modal: {
                Title: 'プロンプトリスト',
                Add: '新規追加',
    ...
    Store: {
        ...
        Prompt: {
            History: (content: string) =>
                'これは、AI とユーザの過去のチャットを要約した前提となるストーリーです：' + content,
    ...
    Mask: {
        ...
        EditModal: {
            Title: (readonly: boolean) => `キャラクタープリセットを編集 ${readonly ? '（読み取り専用）' : ''}`,
            Download: 'ダウンロード',
            Clone: '複製',
        },
        Config: {
